...
class FAddTest extends AnyFreeSpec with ChiselScalatestTester {
  for (stages <- 1 to 5) {
    s"FAdd of ${stages} stages should work" in {
      test(new FAdd(FloatS, 2, stages))
